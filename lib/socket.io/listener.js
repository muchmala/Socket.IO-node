...
Listener.prototype.broadcastExcept = Listener.prototype.broadcast;

Listener.prototype.broadcastOnly = function(message, clients){
  var keys = (Array.isArray(clients))? clients : [clients]
    , len = keys.length
    , key;
...
Listener.prototype.broadcastToChannel = function(channel, message, except){
...
Listener.prototype._onClientDisconnect = function(client){
  ...
  this._deleteAllClientsSubscriptions(client);
...
Listener.prototype._onNewChannelSubscription = function(channel, client) {
  if (!(channel in this.channels)) {
      this.channels[channel] = {};
  }

  this.channels[channel][client.sessionId] = '';
  this._emitChannelSubscriptionEvent(channel);
  this.log('Client '+ client.sessionId +' subscribed to channel ' + channel);
};

Listener.prototype._onDroppedChannelSubscription = function(channel, client) {
  delete this.channels[channel][client.sessionId];

  this._emitChannelSubscriptionEvent(channel);
  this.log('Client '+ client.sessionId +' unsubscribed from channel ' + channel);
};

Listener.prototype._emitChannelSubscriptionEvent = function(channel) {
  this.emit('channelSubscription', channel, Object.keys(this.channels[channel]));
};

Listener.prototype._deleteAllClientsSubscriptions = function(client) {
  var channels = Object.keys(client.channels);

  for (var i = channels.length; i--;) {
    if (channels[i] in this.channels &&
        client.sessionId in this.channels[channels[i]]) {

      this._onDroppedChannelSubscription(channels[i], client);
    }
  }
};
